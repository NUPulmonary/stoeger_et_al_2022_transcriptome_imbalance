...
def mirdb(taxon_id):
    ...
    annotations. All the targets in miRDB were predicted by a bioinformatics
    tool, MirTarget, which was developed by analyzing thousands of
    miRNA-target interactions from high-throughput sequencing
    experiments. Common features associated with miRNA target binding
    have been identified and used to predict miRNA targets with machine
    learning methods. miRDB hosts predicted miRNA targets in five species:
    ...
    taxon_code = {       # may extend to include more animals
        10090: 'mmu-',
        9606: 'hsa-'
    }

    p = inout.get_path(
        'extra_geisen', 'datasets/miRDB_v5.0_prediction_result.txt')
    df = pd.read_table(p, names=['mirna', 'rna_ncbi', 'score'])
    f = df['mirna'].str.startswith(taxon_code[taxon_id])
    df = df.loc[f, ['mirna', 'rna_ncbi']]

    p = inout.get_path('geisen', 'ncbi/gene2ensembl.gz')
    rna_info = pd.read_csv(p)
    rna_info = rna_info[rna_info['taxon_ncbi'] ==
                        taxon_id][['gene_ncbi', 'rna_ncbi.version']]
    rna_info['rna_ncbi.version'] = rna_info['rna_ncbi.version'].str.replace(
        r'\.[0-9]+', '')
    rna_info = rna_info.rename(columns={'rna_ncbi.version': 'rna_ncbi'})

    df = pd.merge(df, rna_info)
    df = df[['mirna', 'gene_ncbi']].drop_duplicates()
    df['mirna'] = df['mirna'].str.replace(taxon_code[taxon_id], '')

    df = df.drop_duplicates()
    df.loc[:, 'is_present'] = True
    df = df.pivot(index='gene_ncbi', columns='mirna',
                  values='is_present')
    df.columns.name = ''
