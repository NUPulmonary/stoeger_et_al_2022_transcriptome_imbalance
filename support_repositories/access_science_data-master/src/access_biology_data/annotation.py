...
def biosystems(taxon_id=None, databases='all'):
    ...
    Input:
        taxon_id    int, or list of ints, or 'all'
    ...
    elif isinstance(taxon_id, numbers.Number):
        multiple_taxa = False
        taxa_to_load = [taxon_id]
    elif taxon_id == 'all':
        multiple_taxa = True
        # taxa_to_load can not yet be defined as different for databases
    elif len(taxon_id) > 1 and not isinstance(taxon_id, str):
        multiple_taxa = True
        taxa_to_load = taxon_id

        for t in taxa_to_load:
            if not isinstance(t, numbers.Number):
                raise ValueError('List must only contain numbers')
    else:
        raise ValueError(
            'Some input argument is unexpected for current code.')
    ...
    for d in databases:
        ...
        if taxon_id == 'all':
            ...
            h = os.listdir(p)
            h = pd.Series(h)
            h = h[h.str.contains('taxon_[0-9]*.csv.gz')]
            taxa_to_load = sorted(
                list(h.str.extract(
                    'taxon_([0-9]*).csv.gz', expand=False).astype(
                    int).drop_duplicates().values))

        agg_over_taxa = []
        database_exists_for_a_taxon = False

        for taxon in taxa_to_load:
            p = inout.get_path(
                'geisen',
                'ncbi/biosystems/' +
            ...
            if os.path.exists(p):
                ...
                agg_over_taxa.append(df)
                database_exists_for_a_taxon = True
            elif not multiple_taxa:
                print('Biosystem {} is not defined for taxon {}'.format(
                    d, taxon))

        if database_exists_for_a_taxon:
            df = pd.concat(agg_over_taxa, sort=True).reset_index(drop=True)
