...
def biogrid(taxon_id, usecols=None):
    """
    Imports BioGrid database

    Input:
        taxon_id    int, or 'all'
        usecols     Optional list of columns to be imported

    Output:
        df_biogrid
    """

    def _load_one_biogrid(taxon_id, usecols):
        p = inout.get_path(
            'geisen',
    ...
    if taxon_id is not 'all':
        df = _load_one_biogrid(taxon_id, usecols)
    else:
        agg = []
        p = inout.get_path('geisen', 'biogrid/*')
        s = pd.Series(glob.glob(p))
        taxa = s.str.extract('_taxon_([0-9]*).').dropna()[0].unique()

        for taxon in taxa:
            agg.append(_load_one_biogrid(taxon, usecols))
        df = pd.concat(agg)
