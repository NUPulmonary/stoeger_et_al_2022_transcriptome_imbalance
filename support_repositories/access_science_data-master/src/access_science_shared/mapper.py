...
def rna_ensembl_2_gene_ncbi(df, how):
    ...
    p_mapper = _get_geisen_path('ncbi/gene2ensembl.gz')
    if not os.path.exists(p_mapper):
        raise EnvironmentError(
...
def rna_ncbi_2_gene_ncbi(df, how):
    """
    Maps ensembl transcript ID to NCBI (Entrez) gene IDs.
    ...
    id_name = 'rna_ncbi'  # Science of Biology nomenclature
    is_column, is_index = _check_for_presence(
        df,
        id_name,
        require_presence=True)
    ...
    if not os.path.exists(p_mapper):
        raise EnvironmentError(
            'rna_ncbi_2_gene_ncbi() requires gene2ensembl')
    mapper = pd.read_csv(
        p_mapper,
        usecols=[
            'gene_ncbi',
            'rna_ncbi.version']).drop_duplicates()

    if is_index:
        df = df.reset_index()

    mapper['rna_ncbi'] =  mapper['rna_ncbi.version'].str.replace(r'\.[0-9]+','')
    mapper = mapper.drop('rna_ncbi.version', axis=1)
...
def uniprot_protein_2_gene_ncbi(df, how):
    ...
    p_mapper = _get_geisen_path('uniprot/uniprot_id_mapper.h5')
    if not os.path.exists(p_mapper):
        raise EnvironmentError(
...
def _check_for_presence(df, id_name, require_presence=False):
    ...
    if require_presence:  # optional input
        if (not is_column) & (not is_index):
            raise ValueError(
                'No {} defined in dataframe to be mapped.'.format(
                    id_name))

    if (is_column) & (is_index):
        raise ValueError(
            'Found {} in index and in column.'.format(
                id_name))
