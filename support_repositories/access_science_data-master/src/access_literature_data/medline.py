...
def select_medline_wos_records(
    ...
    Input:
        columns_sql     string about columns that should be loaded
        years_range     tuple or 'all, or range;
                            optional; range indicating years
    ...
    def _get_citation_years(years):
        """
        tuple to define range

        Input:
            years    range, or tuble with (start_year, first_excluded year)
        Output:
        ...
        if isinstance(years, range):
            ...
            vec = range(years[0], years[1])
        else:
            raise ValueError(
                'Please provide range of years.')
        sql_sub_string = ''
        for v in vec:
    ...
    # clean up unnecessary fields that occur when
    # importing all citations
    if years_range == 'all':
        df = df.drop('index', 1)

        if 'citations.ut' not in columns_sql:
            df = df.drop('ut', 1)
