...
overview <- read.csv('/Users/tstoeger/Dropbox/aging_map_paper/dynamic/tstoeger/181022_inclusive_de/sample_meta.csv')
...
setwd('/Users/tstoeger/Dropbox/aging_map_paper/dynamic/tstoeger/181022_inclusive_de/mockquestmon2_counts/')
  ...
  for (j in ages){ #j=9
    ...
    if(length(unique(ttable$pfu)) != 1 & length(combos) >= 1){
      for (k in combos) {
        ...
        write.csv(res1,paste0("/Users/tstoeger/Dropbox/aging_map_paper/dynamic/tstoeger/181022_inclusive_de/DE/Age/",i,"_Age_",j,"_pfu_",paste(k,collapse = " "),"_DE.csv"),row.names = T)
      } 
  ...
  for (j in flus){ #j=0
    ...
    if(length(unique(ttable$age)) != 1 & length(combos) >= 1){
      for (k in combos) {
        ...
        res1 <- res1[order(-res1$up, -res1$dn, res1$padj),]
        colnames(res1)[1] <- "Symbol"
        write.csv(res1,paste0("/Users/tstoeger/Dropbox/aging_map_paper/dynamic/tstoeger/181022_inclusive_de/DE/Flu/",i,"_pfu_",j,"_ages_",paste(k,collapse = " "),"_DE.csv"),row.names = T)
      }
    } else {print(paste(i," pfu ",j," has only one age condition"))}
  }
}
#
savehistory("/Users/tstoeger/Dropbox/aging_map_paper/dynamic/tstoeger/181022_inclusive_de/DE/datalog.txt")
